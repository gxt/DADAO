...
class WumingScratchpadAdapter(implicit p: Parameters, implicit val wumingConf: WumingCoreParams) extends CoreModule {
  ...
  // The expression below convert TileLink size and signedness to Wuming type.
...
class WumingRequestRouter(cacheAddress: AddressSet)(implicit val conf: WumingCoreParams) extends Module {
  val io = IO(new Bundle() {
