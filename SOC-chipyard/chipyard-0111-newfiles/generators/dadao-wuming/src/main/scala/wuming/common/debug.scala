...
/** Structure to define the top-level DMI interface
  ...
  */
class DMIIO(implicit val conf: WumingCoreParams) extends Bundle {
...
class SimDTM(implicit val conf: WumingCoreParams) extends BlackBox {
...
class DebugDPath(implicit val conf: WumingCoreParams) extends Bundle
{
  // REG access
...
class DebugCPath(implicit val conf: WumingCoreParams) extends Bundle
{
  val halt = Output(Bool())
}

class DebugIo(implicit val conf: WumingCoreParams) extends Bundle
{
...
class DebugModule(implicit val conf: WumingCoreParams) extends Module {
