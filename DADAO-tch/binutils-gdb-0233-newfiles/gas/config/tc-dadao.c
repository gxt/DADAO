...
extern const dadao_opc_info_t dadao_opc_info[128];
...
{
  int count;
  const dadao_opc_info_t *opcode;
  opcode_hash_control = hash_new ();

  /* Insert names into hash table.  */
  for (count = 0, opcode = dadao_form1_opc_info; count++ < 64; opcode++)
    hash_insert (opcode_hash_control, opcode->name, (char *) opcode);

  for (count = 0, opcode = dadao_form2_opc_info; count++ < 4; opcode++)
    hash_insert (opcode_hash_control, opcode->name, (char *) opcode);

  for (count = 0, opcode = dadao_form3_opc_info; count++ < 10; opcode++)
    hash_insert (opcode_hash_control, opcode->name, (char *) opcode);
...
md_assemble (char *str)
{
  ...
  *op_end = 0;
  ...
  opcode = (dadao_opc_info_t *) hash_find (opcode_hash_control, op_start);
  *op_end = pend;
	      ...
	      valueT * valP ATTRIBUTE_UNUSED, segT seg ATTRIBUTE_UNUSED)
{
  ...
  switch (fixP->fx_r_type)
    ...
    case BFD_RELOC_DADAO_10_PCREL:
      if (!val)
	break;
      if (val < -1024 || val > 1022)
	as_bad_where (fixP->fx_file, fixP->fx_line,
                      _("pcrel too far BFD_RELOC_DADAO_10"));
