...
int
print_insn_dadao (bfd_vma addr, struct disassemble_info * info)
{
  int length = 2;
  int status;
  stream = info->stream;
  ...
  if ((iword & (1<<15)) == 0)
    {
      ...
      opcode = &dadao_form1_opc_info[iword >> 8];
      switch (opcode->itype)
	{
	case DADAO_F1_NARG:
  ...
  else if ((iword & (1<<14)) == 0)
    {
      ...
      opcode = &dadao_form2_opc_info[(iword >> 12) & 3];
      switch (opcode->itype)
	{
	case DADAO_F2_A8V:
	  fpr (stream, "%s\t%s, 0x%x",
  ...
  else
    {
      ...
      opcode = &dadao_form3_opc_info[(iword >> 10) & 15];
      switch (opcode->itype)
	{
	case DADAO_F3_PCREL:
	  fpr (stream, "%s\t", opcode->name);
