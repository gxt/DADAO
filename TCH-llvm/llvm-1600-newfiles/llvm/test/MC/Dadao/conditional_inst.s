! RUN: llvm-mc -arch=dadao -show-encoding -show-inst < %s | FileCheck %s
...
! CHECK-NEXT: fixup A - offset: 0, value: jump1, kind: FIXUP_DADAO_25
! CHECK-NEXT: <MCInst #{{[0-9]+}} BRCC{{$}}
...
! CHECK-NEXT: fixup A - offset: 0, value: jump2, kind: FIXUP_DADAO_25
! CHECK-NEXT: <MCInst #{{[0-9]+}} BRCC{{$}}
...
! CHECK-NEXT:   fixup A - offset: 0, value: .Ltmp0, kind: FIXUP_DADAO_25
! CHECK-NEXT: <MCInst #{{[0-9]+}} BT{{$}}
