...
; RUN: llc -march=dadao < %s -code-model=small  | FileCheck -check-prefix CHECK-SMALL %s
; RUN: not llc -march=dadao < %s -code-model=tiny 2>&1 | FileCheck -check-prefix CHECK-TINY %s
; RUN: not llc -march=dadao < %s -code-model=kernel 2>&1 | FileCheck -check-prefix CHECK-KERNEL %s
