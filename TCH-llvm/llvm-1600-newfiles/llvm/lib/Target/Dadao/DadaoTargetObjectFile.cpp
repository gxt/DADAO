...
static cl::opt<unsigned> SSThreshold(
    "dadao-ssection-threshold", cl::Hidden,
    cl::desc("Small data and bss section threshold size (default=0)"),
    cl::init(0));

void DadaoTargetObjectFile::Initialize(MCContext &Ctx,
...
// section.
bool DadaoTargetObjectFile::isGlobalInSmallSection(
...
// Return true if this global address should be placed into small data/bss
// section.
bool DadaoTargetObjectFile::isGlobalInSmallSection(const GlobalObject *GO,
                                                   const TargetMachine &TM,
...
// kind.
bool DadaoTargetObjectFile::isGlobalInSmallSectionImpl(
    const GlobalObject *GO, const TargetMachine &TM) const {
...
MCSection *DadaoTargetObjectFile::SelectSectionForGlobal(
...
bool DadaoTargetObjectFile::isConstantInSmallSection(const DataLayout &DL,
                                                     const Constant *CN) const {
  return isInSmallSection(DL.getTypeAllocSize(CN->getType()));
}

MCSection *DadaoTargetObjectFile::getSectionForConstant(
