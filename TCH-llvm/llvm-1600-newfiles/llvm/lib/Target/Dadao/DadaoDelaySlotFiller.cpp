//===-- DadaoDelaySlotFiller.cpp - Dadao delay slot filler ----------------===//
//
...
static cl::opt<bool>
    NopDelaySlotFiller("dadao-nop-delay-filler", cl::init(false),
                       cl::desc("Fill Dadao delay slots with NOPs."),
                       cl::Hidden);
...
struct Filler : public MachineFunctionPass {
  ...
  StringRef getPassName() const override { return "Dadao Delay Slot Filler"; }

  bool runOnMachineBasicBlock(MachineBasicBlock &MBB);

  bool runOnMachineFunction(MachineFunction &MF) override {
    const DadaoSubtarget &Subtarget = MF.getSubtarget<DadaoSubtarget>();
...
// createDadaoDelaySlotFillerPass - Returns a pass that fills in delay
// slots in Dadao MachineFunctions
FunctionPass *
llvm::createDadaoDelaySlotFillerPass(const DadaoTargetMachine & /*tm*/) {
  return new Filler();
}
  ...
  for (MachineBasicBlock::instr_iterator I = MBB.instr_begin();
    ...
    if (I->getDesc().hasDelaySlot()) {
      ...
      if (I->getOpcode() == Dadao::RET) {
        ...
        assert(RI->getOpcode() == Dadao::LDW_RI && RI->getOperand(0).isReg() &&
               RI->getOperand(0).getReg() == Dadao::FP &&
               RI->getOperand(1).isReg() &&
               RI->getOperand(1).getReg() == Dadao::FP &&
               RI->getOperand(2).isImm() && RI->getOperand(2).getImm() == -8);
        ++RI;
        assert(RI->getOpcode() == Dadao::ADD_I_LO &&
               RI->getOperand(0).isReg() &&
               RI->getOperand(0).getReg() == Dadao::SP &&
               RI->getOperand(1).isReg() &&
               RI->getOperand(1).getReg() == Dadao::FP);
        ...
        if (!NopDelaySlotFiller && findDelayInstr(MBB, I, J)) {
          ...
          BuildMI(MBB, std::next(I), DebugLoc(), TII->get(Dadao::NOP));
        }
        ++FilledSlots;
      }
