...
/* Let's keep the filehandle array here, since this is a primary
   initializer of it.  */
unsigned char _DADAO_allocated_filehandle[32] =
 {
   1,
   1,
   1,
       ...
       int flags, ...)
{
  ...
  for (fileno = 0;
       fileno < (sizeof (_DADAO_allocated_filehandle) /
		 sizeof (_DADAO_allocated_filehandle[0]));
       fileno++)
    if (_DADAO_allocated_filehandle[fileno] == 0)
      break;

  if (fileno == (sizeof (_DADAO_allocated_filehandle) /
		 sizeof (_DADAO_allocated_filehandle[0])))
    {
      errno = EMFILE;
      return -1;
  ...
  _DADAO_allocated_filehandle[fileno] = 1;
