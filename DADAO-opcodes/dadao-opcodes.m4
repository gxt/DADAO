#	$1		$2	$3	$4	$5	$6	$7	$8	$9
#	name		op-type	maj-op	min-op	fa	fb	fc	fd	insn-type
divert(0)dnl
insn(	add,		rrii,	0x20,	0,	rg,	rg,	s12,	-,	normal)
insn(	addu,		rrii,	0x22,	0,	rg,	rg,	u12,	-,	normal)
insn(	sub,		rrii,	0x24,	0,	rg,	rg,	s12,	-,	normal)
insn(	subu,		rrii,	0x26,	0,	rg,	rg,	u12,	-,	normal)

insn(	_add,		rrri,	0x21,	0,	rg,	rg,	rg,	u6,	normal)
insn(	_addu,		rrri,	0x23,	0,	rg,	rg,	rg,	u6,	normal)
insn(	_sub,		rrri,	0x25,	0,	rg,	rg,	rg,	u6,	normal)
insn(	_subu,		rrri,	0x27,	0,	rg,	rg,	rg,	u6,	normal)

insn(	add.rp,		rrii,	0x28,	0,	rp,	rp,	s12,	-,	normal)
insn(	_add.rp,	rrri,	0x29,	0,	rp,	rp,	rg,	u6,	normal)

insn(	mul,		rrii,	0x30,	0,	rg,	rg,	s12,	-,	normal)
insn(	_mul,		rrri,	0x31,	0,	rg,	rg,	rg,	u6,	normal)

insn(	mulu,		rrrr,	0x35,	0,	rg,	rg,	rg,	rg,	normal)
insn(	div,		rrrr,	0x36,	0,	rg,	rg,	rg,	rg,	normal)
insn(	divu,		rrrr,	0x37,	0,	rg,	rg,	rg,	rg,	normal)

insn(	setwl,		rjii,	0x3A,	0,	rg,	w16,	-,	-,	normal)
insn(	setwk,		rjii,	0x3A,	1,	rg,	w16,	-,	-,	normal)
insn(	setwj,		rjii,	0x3A,	2,	rg,	w16,	-,	-,	normal)
insn(	setwh,		rjii,	0x3A,	3,	rg,	w16,	-,	-,	normal)
insn(	incwl,		rjii,	0x3B,	0,	rg,	w16,	-,	-,	normal)
insn(	incwk,		rjii,	0x3B,	1,	rg,	w16,	-,	-,	normal)
insn(	incwj,		rjii,	0x3B,	2,	rg,	w16,	-,	-,	normal)
insn(	incwh,		rjii,	0x3B,	3,	rg,	w16,	-,	-,	normal)

insn(	slu,		orri,	0x3E,	1,	op,	rg,	rg,	u6,	normal)
insn(	sr,		orri,	0x3E,	2,	op,	rg,	rg,	u6,	normal)
insn(	sru,		orri,	0x3E,	3,	op,	rg,	rg,	u6,	normal)
insn(	not,		orri,	0x3E,	4,	op,	rg,	rg,	u6,	normal)
insn(	_slu,		orrr,	0x3F,	1,	op,	rg,	rg,	rg,	normal)
insn(	_sr,		orrr,	0x3F,	2,	op,	rg,	rg,	rg,	normal)
insn(	_sru,		orrr,	0x3F,	3,	op,	rg,	rg,	rg,	normal)
insn(	_not,		orrr,	0x3F,	4,	op,	rg,	rg,	rg,	normal)

insn(	or,		rrii,	0x40,	0,	rg,	rg,	u12,	-,	normal)
insn(	and,		rrii,	0x42,	0,	rg,	rg,	u12,	-,	normal)
insn(	xor,		rrii,	0x44,	0,	rg,	rg,	u12,	-,	normal)

insn(	_or,		rrri,	0x41,	0,	rg,	rg,	rg,	u6,	normal)
insn(	_and,		rrri,	0x43,	0,	rg,	rg,	rg,	u6,	normal)
insn(	_xor,		rrri,	0x45,	0,	rg,	rg,	rg,	u6,	normal)

insn(	cs.n,		rrrr,	0x60,	0,	rg,	rg,	rg,	rg,	normal)
insn(	cs.nn,		rrrr,	0x61,	0,	rg,	rg,	rg,	rg,	normal)
insn(	cs.z,		rrrr,	0x62,	0,	rg,	rg,	rg,	rg,	normal)
insn(	cs.nz,		rrrr,	0x63,	0,	rg,	rg,	rg,	rg,	normal)
insn(	cs.p,		rrrr,	0x64,	0,	rg,	rg,	rg,	rg,	normal)
insn(	cs.np,		rrrr,	0x65,	0,	rg,	rg,	rg,	rg,	normal)
insn(	cs.od,		rrrr,	0x66,	0,	rg,	rg,	rg,	rg,	normal)
insn(	cs.ev,		rrrr,	0x67,	0,	rg,	rg,	rg,	rg,	normal)

insn(	cmp,		rrii,	0x70,	0,	rg,	rg,	s12,	-,	normal)
insn(	cmpu,		rrii,	0x72,	0,	rg,	rg,	u12,	-,	normal)
insn(	_cmp,		rrri,	0x71,	0,	rg,	rg,	rg,	u6,	normal)
insn(	_cmpu,		rrri,	0x73,	0,	rg,	rg,	rg,	u6,	normal)

insn(	ldb,		rrii,	0x80,	0,	rg,	rp,	s12,	-,	dref)
insn(	ldw,		rrii,	0x82,	0,	rg,	rp,	s12,	-,	dref)
insn(	ldt,		rrii,	0x84,	0,	rg,	rp,	s12,	-,	dref)
insn(	ldo,		rrii,	0x86,	0,	rg,	rp,	s12,	-,	dref)
insn(	ldo.rp,		rrii,	0x88,	0,	rp,	rp,	s12,	-,	dref)
insn(	ldt.rf,		rrii,	0x8A,	0,	rf,	rp,	s12,	-,	dref)
insn(	ldbu,		rrii,	0x90,	0,	rg,	rp,	s12,	-,	dref)
insn(	ldwu,		rrii,	0x92,	0,	rg,	rp,	s12,	-,	dref)
insn(	ldtu,		rrii,	0x94,	0,	rg,	rp,	s12,	-,	dref)
insn(	stb,		rrii,	0xA0,	0,	rg,	rp,	s12,	-,	dref)
insn(	stw,		rrii,	0xA2,	0,	rg,	rp,	s12,	-,	dref)
insn(	stt,		rrii,	0xA4,	0,	rg,	rp,	s12,	-,	dref)
insn(	sto,		rrii,	0xA6,	0,	rg,	rp,	s12,	-,	dref)
insn(	sto.rp,		rrii,	0xA8,	0,	rp,	rp,	s12,	-,	dref)
insn(	stt.rf,		rrii,	0xAA,	0,	rf,	rp,	s12,	-,	dref)

insn(	_ldb,		rrri,	0x81,	0,	rg,	rp,	rg,	u6,	dref)
insn(	_ldw,		rrri,	0x83,	0,	rg,	rp,	rg,	u6,	dref)
insn(	_ldt,		rrri,	0x85,	0,	rg,	rp,	rg,	u6,	dref)
insn(	_ldo,		rrri,	0x87,	0,	rg,	rp,	rg,	u6,	dref)
insn(	_ldo.rp,	rrri,	0x89,	0,	rp,	rp,	rg,	u6,	dref)
insn(	_ldt.rf,	rrri,	0x8B,	0,	rf,	rp,	rg,	u6,	dref)
insn(	_ldbu,		rrri,	0x91,	0,	rg,	rp,	rg,	u6,	dref)
insn(	_ldwu,		rrri,	0x93,	0,	rg,	rp,	rg,	u6,	dref)
insn(	_ldtu,		rrri,	0x95,	0,	rg,	rp,	rg,	u6,	dref)
insn(	_stb,		rrri,	0xA1,	0,	rg,	rp,	rg,	u6,	dref)
insn(	_stw,		rrri,	0xA3,	0,	rg,	rp,	rg,	u6,	dref)
insn(	_stt,		rrri,	0xA5,	0,	rg,	rp,	rg,	u6,	dref)
insn(	_sto,		rrri,	0xA7,	0,	rg,	rp,	rg,	u6,	dref)
insn(	_sto.rp,	rrri,	0xA9,	0,	rp,	rp,	rg,	u6,	dref)
insn(	_stt.rf,	rrri,	0xAB,	0,	rf,	rp,	rg,	u6,	dref)

insn(	br.n,		riii,	0xC0,	0,	rg,	s18,	-,	-,	condbranch)
insn(	br.nn,		riii,	0xC1,	0,	rg,	s18,	-,	-,	condbranch)
insn(	br.z,		riii,	0xC2,	0,	rg,	s18,	-,	-,	condbranch)
insn(	br.nz,		riii,	0xC3,	0,	rg,	s18,	-,	-,	condbranch)
insn(	br.p,		riii,	0xC4,	0,	rg,	s18,	-,	-,	condbranch)
insn(	br.np,		riii,	0xC5,	0,	rg,	s18,	-,	-,	condbranch)
insn(	br.od,		riii,	0xC6,	0,	rg,	s18,	-,	-,	condbranch)
insn(	br.ev,		riii,	0xC7,	0,	rg,	s18,	-,	-,	condbranch)

insn(	geta,		riii,	0xCA,	0,	rg,	s18,	-,	-,	geta)
insn(	call,		iiii,	0xD0,	0,	s24,	-,	-,	-,	jsr)
insn(	_call,		rrii,	0xD1,	0,	rg,	rp,	s12,	-,	jsr)
insn(	jump,		iiii,	0xD2,	0,	s24,	-,	-,	-,	branch)
insn(	_jump,		rrii,	0xD3,	0,	rg,	rp,	s12,	-,	branch)

insn(	swym,		oiii,	0xDA,	0,	op,	u18,	-,	-,	normal)
insn(	mov.rp,		orr0,	0xDA,	1,	op,	rp,	rp,	-,	normal)
insn(	mov.rv,		orr0,	0xDA,	3,	op,	rv,	rv,	-,	normal)
insn(	put.rp,		orr0,	0xDA,	0x11,	op,	rp,	rg,	-,	normal)
insn(	put.rv,		orr0,	0xDA,	0x13,	op,	rv,	rg,	-,	normal)
insn(	get.rp,		orr0,	0xDA,	0x21,	op,	rg,	rp,	-,	normal)
insn(	get.rv,		orr0,	0xDA,	0x23,	op,	rg,	rv,	-,	normal)
insn(	nop,		o000,	0xDA,	0x36,	op,	-,	-,	-,	normal)

insn(	ret,		o000,	0xDA,	0x37,	op,	-,	-,	-,	jsr)
insn(	trip,		oiii,	0xDA,	0x38,	op,	u18,	-,	-,	jsr)
insn(	trap,		oiii,	0xDA,	0x3F,	op,	u18,	-,	-,	jsr)

insn(	fcmp,		orrr,	0xDB,	1,	op,	rg,	rf,	rf,	normal)
insn(	fun,		orrr,	0xDB,	2,	op,	rg,	rf,	rf,	normal)
insn(	feql,		orrr,	0xDB,	3,	op,	rg,	rf,	rf,	normal)
insn(	fcmpe,		orrr,	0xDB,	17,	op,	rg,	rf,	rf,	normal)
insn(	fune,		orrr,	0xDB,	18,	op,	rg,	rf,	rf,	normal)
insn(	feqle,		orrr,	0xDB,	19,	op,	rg,	rf,	rf,	normal)

insn(	ft_add,		orrr,	0x2A,	1,	op,	rf,	rf,	rf,	normal)
insn(	ft_sub,		orrr,	0x2A,	2,	op,	rf,	rf,	rf,	normal)
insn(	ft_mul,		orrr,	0x2A,	3,	op,	rf,	rf,	rf,	normal)
insn(	ft_div,		orrr,	0x2A,	4,	op,	rf,	rf,	rf,	normal)

insn(	ft_abs,		orr0,	0x2A,	8,	op,	rf,	rf,	-,	normal)
insn(	ft_neg,		orr0,	0x2A,	9,	op,	rf,	rf,	-,	normal)
insn(	ft_sqrt,	orr0,	0x2A,	10,	op,	rf,	rf,	-,	normal)
insn(	ft_mov,		orr0,	0x2A,	11,	op,	rf,	rf,	-,	normal)
insn(	ft_get,		orr0,	0x2A,	12,	op,	rg,	rf,	-,	normal)
insn(	ft_put,		orr0,	0x2A,	13,	op,	rf,	rg,	-,	normal)
insn(	ft_2fo,		orr0,	0x2A,	14,	op,	rf,	rf,	-,	normal)
insn(	ft_i2f,		orr0,	0x2A,	15,	op,	rg,	rf,	-,	normal)
insn(	ft_u2f,		orr0,	0x2A,	16,	op,	rg,	rf,	-,	normal)
insn(	ft_f2i,		orr0,	0x2A,	17,	op,	rf,	rg,	-,	normal)
insn(	ft_f2u,		orr0,	0x2A,	18,	op,	rf,	rg,	-,	normal)

insn(	ftc.un,		orrr,	0x2A,	32,	op,	rg,	rf,	rf,	normal)
insn(	ftc.or,		orrr,	0x2A,	33,	op,	rg,	rf,	rf,	normal)
insn(	ftc.eq,		orrr,	0x2A,	34,	op,	rg,	rf,	rf,	normal)
insn(	ftc.ne,		orrr,	0x2A,	35,	op,	rg,	rf,	rf,	normal)
insn(	ftc.lt,		orrr,	0x2A,	36,	op,	rg,	rf,	rf,	normal)
insn(	ftc.le,		orrr,	0x2A,	37,	op,	rg,	rf,	rf,	normal)
insn(	ftc.gt,		orrr,	0x2A,	38,	op,	rg,	rf,	rf,	normal)
insn(	ftc.ge,		orrr,	0x2A,	39,	op,	rg,	rf,	rf,	normal)

insn(	fo_add,		orrr,	0x2B,	1,	op,	rf,	rf,	rf,	normal)
insn(	fo_sub,		orrr,	0x2B,	2,	op,	rf,	rf,	rf,	normal)
insn(	fo_mul,		orrr,	0x2B,	3,	op,	rf,	rf,	rf,	normal)
insn(	fo_div,		orrr,	0x2B,	4,	op,	rf,	rf,	rf,	normal)

insn(	fo_abs,		orr0,	0x2B,	8,	op,	rf,	rf,	-,	normal)
insn(	fo_neg,		orr0,	0x2B,	9,	op,	rf,	rf,	-,	normal)
insn(	fo_sqrt,	orr0,	0x2B,	10,	op,	rf,	rf,	-,	normal)
insn(	fo_mov,		orr0,	0x2B,	11,	op,	rf,	rf,	-,	normal)
insn(	fo_get,		orr0,	0x2B,	12,	op,	rg,	rf,	-,	normal)
insn(	fo_put,		orr0,	0x2B,	13,	op,	rf,	rg,	-,	normal)
insn(	fo_2ft,		orr0,	0x2B,	14,	op,	rf,	rf,	-,	normal)
insn(	fo_i2f,		orr0,	0x2B,	15,	op,	rg,	rf,	-,	normal)
insn(	fo_u2f,		orr0,	0x2B,	16,	op,	rg,	rf,	-,	normal)
insn(	fo_f2i,		orr0,	0x2B,	17,	op,	rf,	rg,	-,	normal)
insn(	fo_f2u,		orr0,	0x2B,	18,	op,	rf,	rg,	-,	normal)

insn(	foc.un,		orrr,	0x2B,	32,	op,	rg,	rf,	rf,	normal)
insn(	foc.or,		orrr,	0x2B,	33,	op,	rg,	rf,	rf,	normal)
insn(	foc.eq,		orrr,	0x2B,	34,	op,	rg,	rf,	rf,	normal)
insn(	foc.ne,		orrr,	0x2B,	35,	op,	rg,	rf,	rf,	normal)
insn(	foc.lt,		orrr,	0x2B,	36,	op,	rg,	rf,	rf,	normal)
insn(	foc.le,		orrr,	0x2B,	37,	op,	rg,	rf,	rf,	normal)
insn(	foc.gt,		orrr,	0x2B,	38,	op,	rg,	rf,	rf,	normal)
insn(	foc.ge,		orrr,	0x2B,	39,	op,	rg,	rf,	rf,	normal)
