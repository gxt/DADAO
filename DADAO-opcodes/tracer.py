...
n_set_instructions = 0   # Total set instructions retired while collecting_stats == True
...
if args.ucode:
    ...
    regex = "Cyc=([ 0-9]+) \[([01])\] pc=\[([0-9a-f]+)\] inst=\[([0-9a-f]+)\] W\[([0-9a-f]+)\]\[([01])\] Op1=\[([0-9a-f]+)\] Op2=\[([0-9a-f]+)\] ([SKFH ])([BJREM ])([X ]) \t([a-z0-9]+)\t([ a-z0-9-,.()]+)"
    groupmap = {
        "cycle" : 1,
        "retire" : 2,
        "pc" : 3,
...
for line in args.file:
    ...
    if p:
        ...
        if args.ucode:
            ...
            op1_data = int(extract(p, "op1_data"), 16)  # Register operand 1 data
            op2_data = int(extract(p, "op2_data"), 16)  # Register operand 2 data
        ...
        if collecting_stats:
            if retire:
                n_instructions += 1
                if inst.opcode in BRJMP_OPCODES:
                ...
                elif inst.opcode in SET_OPCODES:
                    n_set_instructions += 1
                else:
...
""".format(cpi=n_cycles / n_instructions,
           ...
           set=100 * n_set_instructions / n_instructions,
